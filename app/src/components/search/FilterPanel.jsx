export default function FilterPanel({ value, onChange }) {
  ...
  return (
    <div className="space-y-4">
      <h2 className="font-semibold text-[#111]">Filters</h2>

      <div>
        <label className="block text-xs font-medium text-[#5a5a55] mb-1">Search</label>
        <input value={value.q ?? ''} onChange={(e) => set('q', e.target.value)}
          placeholder="Address, city, owner…"
          className="w-full rounded-md border border-black/[0.1] px-2 py-1.5 text-sm" />
      </div>

      <div className="grid grid-cols-2 gap-2">
        <input placeholder="City" value={value.city ?? ''} onChange={(e) => set('city', e.target.value)}
          className="rounded-md border border-black/[0.1] px-2 py-1.5 text-sm" />
      ...
      <div>
        <label className="block text-xs font-medium text-[#5a5a55] mb-1">Type</label>
      ...
      <div className="grid grid-cols-2 gap-2">
        <div>
          <label className="block text-xs text-[#5a5a55] mb-1">Beds min</label>
          <input type="number" value={value.beds_min ?? ''}
            onChange={(e) => set('beds_min', +e.target.value || undefined)}
            className="w-full rounded-md border border-black/[0.1] px-2 py-1.5 text-sm" />
        </div>
        <div>
          <label className="block text-xs text-[#5a5a55] mb-1">Baths min</label>
      ...
      <div className="grid grid-cols-2 gap-2">
        <div>
          <label className="block text-xs text-[#5a5a55] mb-1">SqFt min</label>
          <input type="number" value={value.sqft_min ?? ''}
            onChange={(e) => set('sqft_min', +e.target.value || undefined)}
            className="w-full rounded-md border border-black/[0.1] px-2 py-1.5 text-sm" />
        </div>
        <div>
          <label className="block text-xs text-[#5a5a55] mb-1">SqFt max</label>
      ...
      <div className="grid grid-cols-2 gap-2">
        <div>
          <label className="block text-xs text-[#5a5a55] mb-1">Value min</label>
          <input type="number" value={value.value_min ?? ''}
            onChange={(e) => set('value_min', +e.target.value || undefined)}
            className="w-full rounded-md border border-black/[0.1] px-2 py-1.5 text-sm" />
        </div>
        <div>
          <label className="block text-xs text-[#5a5a55] mb-1">Value max</label>
      ...
      <button onClick={() => onChange({ per_page: value.per_page })}
        className="w-full py-2 text-sm text-[#5a5a55] hover:text-[#111]">
        Clear all
      </button>
    </div>
  );
